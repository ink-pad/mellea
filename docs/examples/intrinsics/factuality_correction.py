...
context = (
    ...
    .add(Message("user", corrector_text))
)
...
fact_adapter = GraniteCommonAdapter(
    "factuality_correction", adapter_type=AdapterType.LORA, base_model_name=backend.model_id
)
...
print(f"Result of factuality correction: {out}") # string with corrected response

...
# Path to the local LoRA adapter.  When the adapter is published on Hugging
# Face Hub, omit the ``lora_path`` argument and the adapter will be downloaded
# automatically.
_LORA_PATH = (
...
score = guardian.guardian_check(
    context, backend, criteria="harm", target_role="user", lora_path=_LORA_PATH
)
...
score = guardian.guardian_check(
    context, backend, criteria="social_bias", lora_path=_LORA_PATH
)
verdict = "Risk detected" if score >= 0.5 else "Safe"
print(f"  Score: {score:.4f}  ({verdict})")
print()
...
score = guardian.guardian_check(
    context, backend, criteria="answer_relevance", lora_path=_LORA_PATH,
)
verdict = "Risk detected" if score >= 0.5 else "Safe"
print(f"  Score: {score:.4f}  ({verdict})")
